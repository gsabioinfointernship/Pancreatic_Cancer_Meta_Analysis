library(genekitr)
library(tidyverse)
library(rio)

# Create output directory
dir.create("outputs/DESeq2/Annotated", showWarnings = FALSE, recursive = TRUE)

# Define all GEO dataset IDs
geo_ids <- c("GSE130688", "GSE136569", "GSE171485", "GSE196009",
             "GSE211398", "GSE280271", "GSE293744")

# Process each dataset
for (geo_id in geo_ids) {

  # Step 1: Load DESeq2 results
  input_file <- paste0("outputs/DESeq2/", geo_id, ".csv")
  deseq_results <- import(input_file)

  # Step 2: Convert Gene_ID to character
  deseq_results <- deseq_results |>
    mutate(Gene_ID = as.character(Gene_ID))

  # Step 3: Get gene information from genekitr
  gene_info <- genInfo(
    id = deseq_results$Gene_ID,
    org = "hs",
    unique = TRUE,
    keepNA = FALSE
  )

  # Step 4: Join gene information with DESeq2 results
  annotated <- deseq_results |>
    left_join(gene_info, by = c("Gene_ID" = "input_id"))

  # Step 5: Rename columns
  annotated <- annotated |>
    rename(
      Gene_Symbol = symbol,
      Gene_Description = gene_name,
      Gene_Biotype = gene_biotype
    )

  # Step 6: Reorder columns
  annotated <- annotated |>
    select(Gene_ID, Gene_Symbol, Gene_Description, Gene_Biotype, everything())

  # Step 7: Filter for protein-coding genes only
  annotated <- annotated |>
    filter(Gene_Biotype == "protein_coding")

  # Step 8: Export annotated results
  output_file <- paste0("outputs/DESeq2/Annotated/", geo_id, ".csv")
  export(annotated, output_file)
}
