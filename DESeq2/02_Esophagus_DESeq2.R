...
dir.create("outputs/esophagus/DESeq2", showWarnings = FALSE, recursive = TRUE)
dir.create("outputs/esophagus/PCA", showWarnings = FALSE, recursive = TRUE)
...
for (geo_id in geo_ids) {
  ...
  count_file <- paste0("data/esophagus/raw_counts/", geo_id, "_raw_counts.tsv")
  if (!file.exists(count_file)) {
    count_file <- paste0("data/esophagus/raw_counts/", geo_id, "_raw_counts.csv")
  }
  count_data_raw <- import(count_file)
  colnames(count_data_raw)[1] <- "GeneID"

  # Step 2: Load metadata
  metadata <- import(paste0("data/esophagus/metadata/", geo_id, "_metadata.csv"))
  ...
  output_file <- paste0("outputs/esophagus/DESeq2/", geo_id, ".csv")
  ...
  pca_dir <- paste0("outputs/esophagus/PCA/", geo_id)
