...
dir.create("outputs/pancreatic/DESeq2", showWarnings = FALSE, recursive = TRUE)
dir.create("outputs/pancreatic/PCA", showWarnings = FALSE, recursive = TRUE)

# Define all GEO dataset IDs
geo_ids <- c(
  "GSE130688", "GSE136569", "GSE171485", "GSE196009",
...
for (geo_id in geo_ids) {
  ...
  count_file <- paste0("data/pancreatic/raw_counts/", geo_id, "_raw_counts.tsv")
  if (!file.exists(count_file)) {
    count_file <- paste0("data/pancreatic/raw_counts/", geo_id, "_raw_counts.csv")
  }
  count_data_raw <- import(count_file)
  colnames(count_data_raw)[1] <- "GeneID"

  # Step 2: Load metadata
  metadata <- import(paste0("data/pancreatic/metadata/", geo_id, "_metadata.csv"))
  ...
  output_file <- paste0("outputs/pancreatic/DESeq2/", geo_id, ".csv")
  ...
  pca_dir <- paste0("outputs/pancreatic/PCA/", geo_id)
